...
| path | string | ✓ | 対象ファイル（index_scan_dir はディレクトリ）。要望テキストで指定される | .has-udd/documents/skills/harness-query-engine.json |
...
| `index_scan_dir` | ディレクトリ横断で index 集約 | path=ディレクトリ | path=.has-udd/documents/skills |
...
- `.has-udd/documents/`: クエリ対象となる全集約の document.json 配置ディレクトリ（skills / specs / knowledge / agents / coding）
