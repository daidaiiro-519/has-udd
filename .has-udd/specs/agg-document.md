...
構造化された成果物（Document）の一貫性とライフサイクルを表す集約。状態は生成→検証→描画の順にのみ進み、後続版で置換される。
...
| Status | ライフサイクル状態 | enum: CREATED/VALIDATED/RENDERED/SUPERSEDED。値が等しければ等価。遷移は不変条件で守る。 |
...
| ルール | 守り方 | 根拠 |
|---|---|---|
| status は CREATED→VALIDATED→RENDERED→SUPERSEDED の順にのみ進み、逆行・飛ばしをしない | guard | 成果物の状態の一貫性を保つ |
