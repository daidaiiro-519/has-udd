...
コードの配置・レイヤー境界・依存方向に関する判断相談。「このコードはどこに置くべきか」「この依存関係は正しいか」といった具体的な設計判断の相談。サブドメイン分類（中核・一般・補完）が既に分かっている場合はそれも入力に含める。

### 成果物

配置・レイヤー境界に関する判定結果と、判断基準に基づく理由。該当する場合はアンチパターンの指摘とリスク・代替案。
