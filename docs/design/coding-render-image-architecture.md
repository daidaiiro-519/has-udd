...
| repository | application/ports（interface）＋ adapters/outbound（impl） | aggregate の load/save を提供する driven port・**集約1つに1リポジトリ** |
...
| 必須 | 依存（port / repository）は**コンストラクタ注入**で受け取る（生成は合成ルートのみ） |
| 必須 | application 境界は**結果型（Result 等）で成否を返す**・domain は不変条件違反を**ドメイン例外**で表す |
