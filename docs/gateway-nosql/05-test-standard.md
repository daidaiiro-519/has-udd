...
- **JOIN の正しさ**: 参照実装（素朴な多重ループ結合）と `nanodyn-query` の結果が一致。INNER はマッチ 0 件のタプルを出さない・LEFT は必ず左タプルを出し当該入力属性が欠落、を満たす。索引あり経路と scan フォールバック経路で同一結果。**N テーブル（2/3/4段）とエイリアス（自己結合含む）**で不変条件を検証。
