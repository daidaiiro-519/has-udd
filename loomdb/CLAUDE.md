...
JOIN で差別化する。詳細は README.md と docs/00〜06 を参照。
...
7. **テスト計画（機能面・非機能面の拡張）** — `docs/06-test-plan.md` に詳細計画済み。
   **パッケージング公開（項目4）より先に着手する**（ユーザー判断: 「実運用で
   使用できるレベルのテストや確認はできていない」）。実行順序:
   CI パイプライン → 機能面シナリオ拡張 → 障害注入（クラッシュ耐性）→
   性能/サイズ回帰 → fuzz（nightly toolchain 導入可否は着手時に確認）。
   各領域に着手前にユーザーへ未決定事項（閾値・CI 環境）を確認すること。
