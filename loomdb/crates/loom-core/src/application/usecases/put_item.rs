...
//!
//! item をテーブルに書き込む。キー属性は KeySchema に従い item から取り出す。
//! サンプルでは condition 未対応（TODO: ConditionExpression, spec §5.2）。

use crate::domain::{key_codec, AttributeValue, DbError, Item, TableDef};
use crate::ports::StorageEngine;

pub fn put_item<E: StorageEngine>(engine: &E, def: &TableDef, item: &Item) -> Result<(), DbError> {
