...
test("JOIN のページング: limit + startKey で全行を欠落なく回収", () => {
  const db = ordersDb();
  db.createTable({ name: "users", key: { pk: "id" } });
  db.put("users", { id: "u1", name: "Alice" });
  ...
  for (let guard = 0; guard < 10; guard++) {
    ...
    collected.push(...page.rows.map((r) => r["o.orderId"]));
    if (!page.lastEvaluatedKey) break;
    startKey = page.lastEvaluatedKey;
  }
  assert.deepEqual(collected.sort(), ["o1", "o2", "o3"]);
});

test("projection: get/query/scan で取得属性を絞れる（入れ子パス・#name 対応）", () => {
  const db = ordersDb();
  db.put("orders", {
    userId: "u1", orderId: "p1", amount: 5,
    addr: { city: "tokyo", zip: "100" },
  });
  const got = db.get("orders", { userId: "u1", orderId: "p1" }, {
    projection: "addr.city, #a",
    names: { "#a": "amount" },
  });
  assert.deepEqual(got, { addr: { city: "tokyo" }, amount: 5 });
  ...
  for (const item of page.items) {
    assert.deepEqual(Object.keys(item), ["orderId"]);
  }
});

test("運用 API: stats は O(1) の itemCount・compact はエラーなく呼べる", () => {
  const db = ordersDb();
  const s = db.stats("orders");
  assert.equal(s.itemCount, 3);
  assert.ok(s.storageBytes > 0);
  db.delete("orders", { userId: "u1", orderId: "o1" });
  assert.equal(db.stats("orders").itemCount, 2);
  assert.equal(typeof db.compact(), "boolean"); // 回収の有無は redb の判断
});
