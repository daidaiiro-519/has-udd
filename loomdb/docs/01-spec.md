...
- **結合キーの型は S / N / B**（キーに使える型と同じ）。等価判定は順序保存エンコードの一致 = N は数値として等価（"1.0" = "1"）。
- **v1 実行器の範囲**: root は全走査（root への key_condition プッシュダウンは後続）・ページング（§10.7）は後続。probe は ①結合キー=主キー → main 直引き ②索引 pk 一致 → 索引引き ③なし → scan フォールバック＋warnings。
